...
abstract class NoteDao {
  ...
  @Query(
    "SELECT * FROM notes INNER JOIN note_commits ON notes.currentNoteCommitId = note_commits.noteCommitId WHERE notes.parentNoteId IS NULL"
  )
  ...
  ): NoteWithCurrentCommitAndChildNotes {
    // Insert new note.
  ...
  @Transaction
  open fun updateNoteAndInsertNoteCommit(
    note: Note,
    currentNoteCommit: NoteCommit,
  ) {
    // Insert new commit.
